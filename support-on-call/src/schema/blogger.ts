...
/**
 * Service for fetching Blogger post JSON feeds, parsing JSON-LD, recursively resolving
 * @id entities, merging local property overrides, and outputting clean JSON-LD @graph documents.
 */
export class BloggerDataService {
  ...
  /**
   * Fetches a Blogger post's JSON feed using native fetch() and extracts its JSON-LD.
   */
  async fetchPostSchema({
    ...
    try {
      ...
      if (!response.ok) {
        console.error(`HTTP error fetching post: ${response.status}`);
        return null;
      }
      ...
      console.error(`Error fetching post schema for ${blogId}/${postId}:`, e);
      return null;
    }
  }
      ...
      if (typeof idValue === 'string' && idValue.trim().length > 0) {
        ...
        if (blogId && postId) {
          ...
          try {
            ...
            console.error(`Error fetching from full URL ${fullUrl}:`, e);
          }
        }
